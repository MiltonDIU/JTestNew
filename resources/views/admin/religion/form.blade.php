<div class="form-group {{ $errors->has('title') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <input class="form-control" name="title" type="text" id="title" value="{{ $religion->title ?? ''}}" >
        {!! $errors->first('title', '<p class="help-block">:message</p>') !!}
    </div>
</div><div class="form-group {{ $errors->has('alias') ? 'has-error' : ''}}">
    <label for="alias" class="col-md-4 control-label">{{ 'Alias' }}</label>
    <div class="col-md-6">
        <input class="form-control" name="alias" type="text" id="alias" value="{{ $religion->alias ?? ''}}" >
